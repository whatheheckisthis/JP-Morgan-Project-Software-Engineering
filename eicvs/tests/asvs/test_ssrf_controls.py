...
# 
VECTOR_PATH = Path(__file__).resolve().parents[1] / "fixtures/ssrf_vectors.json"


def _requests(category: str) -> list[SsrfRequest]:
    fixture = json.loads(VECTOR_PATH.read_text(encoding="utf-8"))
    items = []
    for vector in fixture[category]:
        payload = vector["payload"]
        items.append(SsrfRequest(vector_id=vector["id"], target_host=payload["target_host"], socket_host=payload["socket_host"], category=category))
    return items
