...
# 
VECTOR_PATH = Path(__file__).resolve().parents[1] / "fixtures/supply_chain_vectors.json"


def _requests(category: str) -> list[SupplyChainRequest]:
    fixture = json.loads(VECTOR_PATH.read_text(encoding="utf-8"))
    items = []
    for vector in fixture[category]:
        payload = vector["payload"]
        items.append(SupplyChainRequest(vector_id=vector["id"], signature_verified=payload["signature_verified"], version_pinned=payload["version_pinned"], category=category))
    return items
