...
CLASS_ID: Final[str] = "auth"


def _fixture_to_requests(fixture: dict) -> list[AuthRequest]:
    requests: list[AuthRequest] = []
    for category in ("positive", "negative", "edge"):
        for vector in fixture[category]:
            payload = vector["payload"]
            requests.append(AuthRequest(vector_id=vector["id"], credential_strength=payload["credential_strength"], mfa_present=payload["mfa_present"], category=category))
    return requests
...
def run(vector_path: str) -> EvidenceRecord:
    ...
    try:
        test_path = Path(__file__).resolve().parents[2] / "tests" / "asvs" / "test_auth_controls.py"
