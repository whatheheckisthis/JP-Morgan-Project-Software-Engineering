...
from eicvs.services.trace.fixture_ssrf_target import SsrfRequest, _map_controls, analyse

CLASS_ID: Final[str] = "ssrf"


def _fixture_to_requests(fixture: dict) -> list[SsrfRequest]:
    requests: list[SsrfRequest] = []
    for category in ("positive", "negative", "edge"):
        for vector in fixture[category]:
            payload = vector["payload"]
            requests.append(SsrfRequest(vector_id=vector["id"], target_host=payload["target_host"], socket_host=payload["socket_host"], category=category))
    return requests
...
def run(vector_path: str) -> EvidenceRecord:
    ...
    try:
        test_path = Path(__file__).resolve().parents[2] / "tests" / "asvs" / "test_ssrf_controls.py"
