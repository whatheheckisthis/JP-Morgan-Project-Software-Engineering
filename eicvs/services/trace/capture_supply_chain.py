...
from eicvs.services.trace.fixture_supply_chain_target import SupplyChainRequest, _map_controls, analyse

CLASS_ID: Final[str] = "supply_chain"


def _fixture_to_requests(fixture: dict) -> list[SupplyChainRequest]:
    requests: list[SupplyChainRequest] = []
    for category in ("positive", "negative", "edge"):
        for vector in fixture[category]:
            payload = vector["payload"]
            requests.append(SupplyChainRequest(vector_id=vector["id"], signature_verified=payload["signature_verified"], version_pinned=payload["version_pinned"], category=category))
    return requests
...
def run(vector_path: str) -> EvidenceRecord:
    ...
    try:
        test_path = Path(__file__).resolve().parents[2] / "tests" / "asvs" / "test_supply_chain_controls.py"
