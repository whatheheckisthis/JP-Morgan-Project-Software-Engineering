...
CLASS_ID: Final[str] = "iam"


def _fixture_to_requests(fixture: dict) -> list[IamRequest]:
    requests: list[IamRequest] = []
    for category in ("positive", "negative", "edge"):
        for vector in fixture[category]:
            payload = vector["payload"]
            requests.append(IamRequest(vector_id=vector["id"], role_hierarchy_allows_escalation=payload["role_hierarchy_allows_escalation"], resource_access_control_present=payload["resource_access_control_present"], category=category))
    return requests
...
def run(vector_path: str) -> EvidenceRecord:
    ...
    try:
        test_path = Path(__file__).resolve().parents[2] / "tests" / "asvs" / "test_iam_controls.py"
